...
//  img_1 = cv::imread("../lion.png", CV_LOAD_IMAGE_COLOR);
  img_1 = cv::imread("../tiger.png", CV_LOAD_IMAGE_COLOR);
  img_1.convertTo(img_1, CV_32F, 1.0/255.0, 0);
  showResult("Lab 2 - Image 1 original", img_1);
//  cv::Mat img_11;
//  double minVal_1, maxVal_1, minVal_11, maxVal_11;
//  cv::minMaxIdx(img_1, &minVal_1, &maxVal_1);
//  cv::minMaxIdx(img_11, &minVal_11, &maxVal_11);
//  cv::normalize(img_11, img_11);

  cv::Mat img_2;
  img_2 = cv::imread("../white_tiger.png", CV_LOAD_IMAGE_COLOR);
  img_2.convertTo(img_2, CV_32F, 1.0/255.0, 0);
  ...
  cv::Mat weights;
  weights = cv::Mat(img_1.size(), CV_32FC3, cv::Scalar(0., 0., 0.));
  //weights(cv::Range(0, weights.rows), cv::Range(0, weights.cols/2)) = cv::Scalar(1., 1., 1.);
  weights(cv::Range(0, weights.rows), cv::Range(weights.cols/2, weights.cols)) = cv::Scalar(1., 1., 1.);
  cv::blur(weights, weights, cv::Size(ramp_width, ramp_width));
