...
PnPPoseEstimator::PnPPoseEstimator(const Eigen::Matrix3d& K)
{
  // Convert to cv::Mat.
  cv::eigen2cv(K, K_);
}
                                        ...
                                        const std::vector<cv::Point3f>& world_points)
{
  ...
  // TODO-3-1: Take a look at the OpenCV documentation for "solvePnPRansac". What is r_vec and t_vec?
  // TODO-3-2: Improve the pose estimation by testing different values for the maximal reprojection error.
  ...
  // TODO-3-3: Use "cv::solvePnP" on inlier points to improve "r_vec" and "t_vec". Use the iterative method with current r_vec and t_vec as initial values.
