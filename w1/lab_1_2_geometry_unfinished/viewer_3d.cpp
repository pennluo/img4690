...
void Viewer3D::addBody(Sophus::SE3d pose_local_body, int img_num)
{
...
void Viewer3D::addCamera(const PerspectiveCameraModel& cameraModel, const cv::Mat& img, int img_num)
{
  // Convert from Eigen::Matrix to cv::Mat
  cv::Mat T_local_body_cv;
  ...
  // Add camera axis.
  viz_win_.showWidget(cam_id.str() + "_axis", cv::viz::WCoordinateSystem(10.0), cv::Affine3d{T_local_body_cv});
}
