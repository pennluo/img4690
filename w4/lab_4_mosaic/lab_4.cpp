...
void lab4()
{
  ...
  // Todo: Step 1: Play with different detectors.
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      // Todo: Step 2: Implement ratio test in extractGoodRatioMatches().
      ...
      if (good_matches.size() >= 10)
      {
        ...
        // Todo: Step 3: Finish estimator by implementing HomographyEstimator::computeReprojectionError().
        // Estimate homography.
        // Measure how long the processing takes.
        start = Clock::now();
        ...
        // Todo: Step 4: Finish mosaic.
        // Insert a downscaled reference image into the mosaic.
        cv::Mat mosaic;
        cv::warpPerspective(ref_image, mosaic, S_cv, frame.size());

        // Warp a mask for the current frame.
        cv::Mat mask = cv::Mat::ones(frame.size(), CV_8UC1);
        cv::Mat mask_warp;
        cv::warpPerspective(mask, mask_warp, S_cv*H_cv, frame.size());
        cv::erode(mask_warp, mask_warp, cv::Mat());

        // Insert the warped current frame into the mosaic using the warped mask.
        cv::Mat frame_warp;
